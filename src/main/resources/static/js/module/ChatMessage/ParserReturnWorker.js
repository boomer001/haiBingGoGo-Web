...
export default class ParserReturnWorker {
    ...
    */
    porserMessages(messages, father) {
        for (let i = 0; i < messages.length; i++) {
            ...
            // console.log('porserMessages:  ', JSON.stringify(messages))
            ...
            if (!message.messageType && message.adaptiveCards) {//如果是正常的聊天消息
                ...
                if (message.sourceAttributions && message.sourceAttributions.length > 0) {
            ...
            } else if (message.messageType === 'InternalSearchQuery') { //如果是搜索消息
                let div = this.getByID(message.messageId, 'div', father, 'InternalSearchQuery');
                this.porserLineTextBlocks(message.text, div);

            } else if (message.messageType === 'InternalSearchResult') { //如果是内部搜索的结果
                // console.log('InternalSearchResult: ', JSON.stringify(message));
                ...
                div.innerHTML = `${message.hiddenText}<br>聊天中断！试试开始新主题？`;

            } else if (message.contentOrigin === 'TurnLimiter') {
                this.addError(message.text);
                this.addError('同主题对话达到限量，试试开始新主题？');
    ...
    */
    porserTextBlock(body, father) {
        // console.log('porserTextBlock:  ',JSON.stringify(body))
        if (!body.size) {
    ...
    */
    ...
    porserSourceAttributions(sourceAttributions, father) {
        ...
        for (let i = 0; i < sourceAttributions.length; i++) {
            ...
            html = html + `<a target="_blank" href="${sourceAttribution.seeMoreUrl}">${i + 1}.&nbsp&nbsp${sourceAttribution.providerDisplayName}</a>`;
        }
