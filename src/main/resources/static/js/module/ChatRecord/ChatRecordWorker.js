...
export default class ChatRecordWorker {
    ...
    constructor(bingChat, chatModeSwitchingWorker, chatSuggestionsWorker, parserReturnWorker, chatRecordDIV, newChatRecordDIV, cleanRecord) {
        ...
        this.deitDiv.classList.add("ChatRecordsListDiv_div");
        this.deitDiv.classList.add("ChatRecordsListDiv_div_edit");
        ...
        this.deitDiv.rename.classList.add("ChatRecordsListDiv_div_input");
        ...
        this.deitDiv.idDiv.classList.add('ChatRecordsListDiv_div_id');

        this.deitDiv.timeDiv = document.createElement('div');
        this.deitDiv.timeDiv.classList.add('ChatRecordsListDiv_div_time');

        this.deitDiv.deleteDiv = document.createElement('div');
        this.deitDiv.deleteDiv.classList.add('ChatRecordsListDiv_div_delete');
    ...
    addRecord(text) {
        ...
        // console.log(text)
        text = this.isEmpty(text) ? '新聊天' : text;
    ...
    updateList() {
        ...
        recordList.forEach((v) => {
            ...
            if (this.userChatRecord && (v.id == this.userChatRecord.id)) {
                ...
                div.innerHTML = `
                    <div class="ChatRecordsListDiv_div_id">${v.id}</div>
                    <div class="ChatRecordsListDiv_div">${v.name}<br>
                    ${v.creationTime}
                    </div> `;
